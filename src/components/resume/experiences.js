  ...
  return (
    <div id={id}>
      ...
      <article>
        ...
        <section>
          一、完成了 BI 工具的反向代理功能，将原来暴露在外网的 BI 集群封闭到防火墙之内，
        ...
        <section>
          ...
          最大的挑战在于要为原来应用里的路由表（每个 URL 所映射的方法）增加一个版本检查，
          确定当前的 URL 所属于哪个版本。拆分版本后提升了用户细分能力，
          为不同需求提供了更佳专业化的服务。
        </section>
        <section>
          三、数据银行粉丝趴项目中的
      ...
      <article>
        ...
        <section>
          ...
          同时实现了视频广告监测 Snap 产品的前期研发工作，因为出色的工作能力，
          职称从 T4 升职到 T5，同时成为了
        ...
        <Row gutter={16} className="products gutter-box">
          ...
          <Col xs={24} lg={8} className="product gutter-box">
            ...
            >
              <section>
                开发过程中经常会碰到那种，需要给某个元素提供 ＋、－ 按钮，可以克隆它的，
                这个组件就是为了解决这个问题，它提供了一套容器，
                把需要克隆的元素套进去便自动增加了 ＋、－ 两个按钮，
                可以很轻松地实现需要的功能。
              </section>
              <section>
                PS: 该组件开发比较早，还是匹配 Bootstrap 2 的。
              </section>
            </ProjectCard>
          </Col>
        </Row>
        <Row className="products">
          <Col xs={24} className="product">
            ...
            >
              ...
              <section>
                该项目从 React 0.12 开始便一直跟踪着最新版本的动态，每周更新所使用的模块，
                开始了去 jQuery 的尝试，同时引入了 Jest 单元测试框架和 travs-ci
                进行前端持续集成 ，力求能够在商业产品中进行应用。
              </section>
              <section>
                目前计划是进行扩展，之前是最简化的编译、发布、测试系统，
                未来将整合一套界面框架和图表，改为变成一套 Dashboard 模板。
              </section>
              <section>
